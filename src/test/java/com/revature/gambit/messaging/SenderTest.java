...
public class SenderTest {
	...
	public void testSend() throws InterruptedException{
		...
		assertThat(received, hasValue(UUIDService.getServiceInstanceIdentifier().toString()+" \""+greeting+"\""));
	...
	public void testSendBatch() throws InterruptedException{
		...
		try {
			batchJSON = MAPPER.writeValueAsString(testBatch);
		} catch (JsonProcessingException e) {
			LOGGER.warn(e.toString());
			e.printStackTrace();
		}
		sender.send(SENDER_TOPIC, testBatch);
		
		//check that the message was recieved
		ConsumerRecord<String, String> received = records.poll(10, TimeUnit.SECONDS);
		//check that batchJSON is not null
		assertThat(batchJSON).isNotNull();
		// Hamcrest Matchers to check the value
		assertThat(received, hasValue(UUIDService.getServiceInstanceIdentifier().toString()+" "+batchJSON));
		// AssertJ Condition to check the key
		assertThat(received).has(key(null));
